...
finite positive step, rejecting violations with \code{ValueError}, and return a
Python float.  The benchmark sweeps logarithmic
steps on a stable classification loss and locates the empirical minimum.
  ...
  \item Invalid coordinate, nonpositive/nonfinite step, or non-scalar function result fails with \code{ValueError}.
  \item The benchmark saves a log--log U-curve and prints the best step/error for this function and dtype. (Trust-based: the test suite does not machine-check benchmark output.)
        \end{itemize}
        \end{evidencebox}
