"""Contract tests for AP-02-016: TensorFlow Cross-Entropy: GradientTape and Logit Semantics.
...
class TensorFlowCrossEntropyTests(unittest.TestCase):
    """Tape and loss-semantics tests. / tape 与损失语义测试。"""
    ...
    def test_loss_reduction_and_gradient_oracle(self) -> None:
        ...
        self.assertEqual(type(report["mean_loss"]), float)
    ...
    def test_rejects_bad_numpy_contract(self) -> None:
        with self.assertRaises(ValueError):
            tensorflow_cross_entropy_probe(np.ones((2, 3), dtype=np.float32), np.array([0, 1]))
        with self.assertRaises(ValueError):
            tensorflow_cross_entropy_probe(np.ones((2, 3)), np.array([0.0, 1.0]))
        with self.assertRaises(ValueError):
            tensorflow_cross_entropy_probe(np.ones((2, 3)), np.array([0, 1], dtype=np.int32))
