...
\code{log\_max} (the real estimate, evaluated as a Python float64 logarithm,
not rounded to the probed dtype), \code{last\_finite}, \code{first\_overflow}, and
...
warning during the probe.
  ...
  \item The benchmark prints a dtype table; there is no single cross-dtype threshold. (Trust-based: the test suite does not machine-check benchmark output.)
        \end{itemize}
        \end{evidencebox}
  ...
  \item Support exactly float16, float32, and float64; reject any other dtype with \code{ValueError}.
        \end{itemize}
