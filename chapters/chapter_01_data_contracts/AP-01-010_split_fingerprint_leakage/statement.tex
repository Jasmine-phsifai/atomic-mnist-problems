...
def cross_split_duplicates(train: np.ndarray, valid: np.ndarray) -> list[tuple[int, int]]:
  ...
  \item The benchmark prints duplicate-pair count, affected-validation count, and leakage rate separately.
        \end{itemize}
        \end{evidencebox}
  ...
  \item Reject rank-zero arrays and incompatible trailing row shapes.
        \end{itemize}
